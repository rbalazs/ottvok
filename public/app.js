...
require(['knockout-3.4.0', 'jquery', 'TripModel', 'ProvisionModel', 'utils',
    'moment',
    'async!https://maps.googleapis.com/maps/api/js?key=AIzaSyC_ZRibN_wYmc-bhLP7L8saRMoitVuUVFU&libraries=places'
    ],
  function (ko, $, TripModel, ProvisionModel, utils, moment) {
    function doProvision(trip, minutesFromNowArg) {
      ...
      $.getJSON(url + timestamp, function (data) {
        ...
        provision.durationInTraffic(data.rows[0].elements[0].duration_in_traffic
          .text);
        provision.timestampVerbose(moment.unix(timestamp).fromNow() +
    ...
    $(document).ready(function () {
      ...
      google.maps.event.addListener(originAddressesAS, 'place_changed',
        function () {
          trip.originAddresses(originAddressesAS.getPlace().formatted_address);
        });

      google.maps.event.addListener(destinationAddressesAS,
        'place_changed',
        function () {
      ...
      $('.do-provision').click(function () {
        ...
        if (typeof trip.destinationAddresses() === 'undefined') {
          trip.destinationAddresses(
            'Budapest Mészáros 1016 Magyarország');
        }
